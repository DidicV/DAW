<?php
  ...
  if($filter_value == 1 || $filter_value == "")
  {
    ...
    {
      $sql = "SELECT * FROM kanban WHERE stare=3 AND (assigned = '$id' OR assigned = 0 OR assigned = '$departament')";
    }
  }
  elseif ($filter_value == 2) 
  {
      $sql = "SELECT * FROM kanban WHERE stare=3 AND assigned = 0 ";
  }
  elseif ($filter_value == 3) 
  {
    $sql = "SELECT * FROM kanban WHERE stare=3 AND assigned = '$departament' ";
  }
  elseif ($filter_value == 4) 
  {
    $sql = "SELECT * FROM kanban WHERE stare=3 AND assigned = '$id' ";
  }
...
<tr>
	<td style="border:1px solid black; padding: 2.5%; background: rgb(210, 214, 218); border-radius: 5px;" width="370px">
      <div style="float: right;">
        <button class="kanban_modf" id="modifica2" data-id="<?php echo $row['txt']; ?>">
